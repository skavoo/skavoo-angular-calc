  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['numero'] && !changes.numero.firstChange) {
      ...
      if (changes.numero.currentValue == null) {
        ...
        // left padding
        for (var i = 0; i < this.cifreIntere - splitted[0].length; i += 1) {
          this.digits.push('\u00A0\u00A0');
        }

        // parte intera
        for (var i = 0; i < splitted[0].length; i += 1) {
        ...
        // parte decimale
        if (splitted[1]) {
          for (var i = 0; i < splitted[1].length; i += 1) {
          ...
          if (splitted[1].length < this.cifreDecimali) {
            for (
              var i = 0;
              i < this.cifreDecimali - splitted[1].length;
              i += 1
            ) {
              this.digits.push('0');
            }
        ...
        // se la parte decimale manca riempi di 0
          ...
          for (var i = 0; i < this.cifreDecimali; i += 1) {
            this.digits.push('0');
          }
        }
      }
